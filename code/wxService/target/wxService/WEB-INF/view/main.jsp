		...
		<section class="Hui-article-box">
			...
			<div id="iframe_box" class="Hui-article">
				<div class="show_iframe">
					...
					<!--queryArticlePanel queryWxMenuPanel-->
				</div>
			</div>
		</section>
						...
						for(idx in res) {
							if(res[idx]["isHide"] == "N") {
								if(res[idx]["menuLevel"] == 1) {
									str += '<dl id="menu-member' + res[idx]["menuCode"] + '" onclick="showChildrenMenu(this)">';
									str += '<dt><i class="Hui-iconfont">&#xe60d;</i> ' + res[idx]["menuTitle"] + '<i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i></dt>';
									str += '<dd>';
...
//									if("素材管理" == res[idx]["menuTitle"]){
//										if($(".menu_dropdown ul").hasClass(res[idx]["parentsCode"])) {
//											str += '<li><a onclick="alertFun();" data-title="' + res[idx]["menuTitle"] + '" href="javascript:void(0)">' + res[idx]["menuTitle"] + '</a></li>';
//										}
//									}else{
										if($(".menu_dropdown ul").hasClass(res[idx]["parentsCode"])) {
											str += '<li><a data-href="' + res[idx]["menuUrl"] + '" data-title="' + res[idx]["menuTitle"] + '" href="javascript:void(0)">' + res[idx]["menuTitle"] + '</a></li>';
										}
//									}
			...
			var updateNetwork = function(type){
				...
				layer.confirm('是否要'+_type+'网络？', {icon: 5, title:'提示'}, function(index){
					...
					jsonDataList.data = {configValue:type};
					var res = showAjaxFun(jsonDataList, "");
			...
			var showChildrenMenu= function(_this) {
				if($(_this).children("dt").hasClass("selected")){
					$(_this).children("dt").removeClass("selected");
					$(_this).children("dd").removeClass("show");
				}else{
					$(_this).children("dt").addClass("selected");
					$(_this).children("dd").addClass("show");
				}
			}
