...
<script>
	...
	var showMenuData = function(title,_menuLevel,_syMenuId){
		$(".menuNameTd").html('<input type="text" value="'+title+'" class="form-control  menuName" style="width: 100%;height: 26px;"/>');//菜单名称
		
		$(".form-control").bind('keyup afterpaste keypress', function() {
	   		this.value=this.value.replace(/(eval)|(<)|(>)|(&lt;)|(&gt;)|(script)|(\/)/g,""); 
	  	});
		var showMenuTier = "";
		switch(_menuLevel){
			case "1":
	...
	var save = function(){
		if($(".menuNameTd").html().trim() != ""){
			parent.layer.confirm('是否修改该信息？', {icon: 5, title:'提示'}, function(index){
				...
				jsonDataList.data = encryptionFun({syMenuId:$(".syMenuId").val(),menuTitle:$(".menuName").val().trim()},userKey.key,userKey.iv);
				jsonDataList.dataType = "text";
				var res = showAjaxFun(jsonDataList,"");
					res =  decryptionFun(res,userKey.key,userKey.iv);
