...
var updateType = function(_this,_syConfigId,_isShow){
	...
	$(_this).parents("tr").children(".configValue").html('<input type="text" style="width:100%;" class="form-control configValueStr" value="'+configValue+'"/>');
...
var update = function(_this,_syConfigId,_isShow){
	var is = layer.confirm("是否确定保存！",function(index){
		var configValueStr = $(_this).parents("tr").children(".configValue").find("input").val().trim();
			...
			jsonDataList.data = encryptionFun(obkList,userKey.key,userKey.iv);
			jsonDataList.dataType = "text";
			var res = showAjaxFun(jsonDataList,"");
				res =  decryptionFun(res,userKey.key,userKey.iv);
			layer.alert(res.message);
